...
@pytest.mark.asyncio
async def test_mistral_provider_generates_response(
...
async def test_mistral_provider_translates_http_errors(
    monkeypatch: pytest.MonkeyPatch,
    status_code: int,
...
@pytest.mark.asyncio
async def test_mistral_provider_translates_generic_http_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    response = make_response(
        400,
...
@pytest.mark.asyncio
async def test_mistral_provider_rejects_invalid_json(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = httpx.Request(
        "POST",
...
@pytest.mark.asyncio
async def test_mistral_provider_rejects_invalid_response(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    response = make_response(
        200,
